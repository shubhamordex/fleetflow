...
export default function Maintenance() {
  ...
  const filteredServices = (() => {
    ...
    return services.filter((s) => {
      const hay =
        `${s.id} ${s.vehicle} ${s.issue} ${s.date} ${s.cost} ${s.status}`.toLowerCase();
                    ...
                    filteredServices.map((s) => (
                      <tr key={s.id} className="border-b hover:bg-gray-50">
                        ...
                        <td className="py-3 px-4">
                          <span
                            className={`px-2 py-1 rounded text-xs ${s.status === "Completed" ? "bg-green-100 text-green-800" : s.status === "New" ? "bg-blue-100 text-blue-800" : "bg-yellow-100 text-yellow-800"}`}
                          >
                            {s.status}
                          </span>
                        </td>
